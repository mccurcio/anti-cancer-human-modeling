...
2. Do any features have skew >= 2.0? Should these features be transformed?  
3.
...
First we need to contenate two files such that we can use them in the following work.

1. /human-comps/human_ctrl_comps.csv
2. /anti-cancer-comps/anti_cancer_comps.csv

To concatenate: `cat *.csv > cancer_human_comps.csv`

However the top line of ONE of the files needs to be extracted from the `anticancer_human_proteins.csv` such that the header from one file is embedded in the middle to concatenated file.
...
2. To determine if dimensionality reduction can allow us to remove unnessary features.

Import data
```{r message=FALSE, warning=FALSE}
cancer_human_comps <- read_csv("1data/cancer_human_comps.csv",
                               col_types = cols(Class = col_factor(levels = c("0","1"))))
colnames(cancer_human_comps)[244] <- "NA" # NOTE: Column X244 should be marked "NA".  "NA" = Asparagine + Alanine
...
- Using `str()`, we see Classes and numerical data is correct and alphebetization is correct.
```{r}
str(cancer_human_comps)
```
...
```{r}
sapply(cancer_human_comps, function(x) sum(is.na(x)))
```
...
chcomp_corr_mat = cor(cancer_human_comps[,c(4:23)], 
                     method = "p") # "p": Pearson test for continous variables

corrplot(abs(chcomp_corr_mat),
         title = "Correlation Plot Of AA Features", 
         method = "square", 
         type = "lower",
         tl.pos = "d",
         cl.lim = c(0, 1),
